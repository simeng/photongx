...
local function init_db()
    ...
    if ngx.shared.config.redis.unix_socket_path then
        ok, err = red:connect("unix:" .. ngx.shared.config.redis.unix_socket_path)
    elseif ngx.shared.config.redis.host then
        local port = 6379
        if ngx.shared.config.redis.port then
            port = ngx.shared.config.redis.port
        end
        ok, err = red:connect(ngx.shared.config.redis.host, port)
    end
    if not ok then
